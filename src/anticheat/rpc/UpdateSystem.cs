...
namespace MalumMenu.anticheat.rpc
{
	internal class UpdateSystem : RpcCheck
	{
		...
		private static void ValidateSwitchSystem(PlayerControl player, MessageReader reader, ref bool blockRpc)
		{
			...
			{
				ConsoleUI.Log($"Blocked switch update from {player.Data.PlayerName} as lights are not currently sabotaged");
				blockRpc = true;
			}
			...
			{
				ConsoleUI.Log($"Blocked switch update from {player.Data.PlayerName} as there is a currently active meeting");
				blockRpc = true;
			}
		}
